...
# ---------------- title auto-fit (measured, not estimated) ----------------
# Usable width of the title textbox, with a safety margin: PowerPoint and
# LibreOffice lay text out slightly differently, so a title measured at 99% of
# the box still wraps when rendered. 5% keeps borderline titles on one line.
TITLE_BOX_IN = 11.75 * 0.95
...
def _title_width_in(text, pt):
    """Rendered width of `text` in Arial Bold at `pt`, in inches."""
    try:
        from PIL import ImageFont
        for path in _ARIAL_BOLD_CANDIDATES:
            if os.path.exists(path):
                f = ImageFont.truetype(path, size=int(pt*4))   # 4x for precision
                return (f.getlength(text)/4.0)/72.0
    except Exception:
        pass
    # conservative fallback if no font file is available
    return len(text)*pt*0.55/72.0

def _fit_title_size(title):
    """Largest house size at which the title still fits on ONE line."""
    for pt in (28, 26, 24, 22, 21):
        if _title_width_in(title, pt) <= TITLE_BOX_IN:
            return pt
    return 20
def mark(key):
    """Record that <key> begins on the NEXT slide to be numbered."""
    SLIDE_MAP[key]=PAGE["n"]+1
FOOTER_TITLE = "Build and Deploy Agentic AI Apps"   # short form — the full title overruns the footer
...
def head(s,title,kicker=None,kcolor=BLUE):
    ...
    # auto-shrink long titles so a wrapped second line never strikes the rule
...
def novaspc_slide():
    ...
    footer(s); return s
# ================================================================
# COMPACT v3 COMPONENTS — added for API/SDK-teaching courses.
# Syntax-highlighted code, annotated code, split code+points, API
# reference tables, comparison matrices, numbered concept stacks and
# a "anatomy" callout. These are what let a technical deck stay
# visual while still showing real code.
# ================================================================
CODE_BG   = RGBColor(0x0B,0x12,0x20)   # near-black navy
CODE_TXT  = RGBColor(0xE6,0xED,0xF3)   # default code foreground
CODE_KW   = RGBColor(0xFF,0x7B,0x72)   # keywords            (red)
CODE_STR  = RGBColor(0xA5,0xD6,0xFF)   # strings             (light blue)
CODE_FN   = RGBColor(0xD2,0xA8,0xFF)   # functions/classes   (violet)
CODE_CMT  = RGBColor(0x8B,0x94,0x9E)   # comments            (grey)
CODE_NUM  = RGBColor(0x79,0xC0,0xFF)   # numbers/constants   (blue)

_KEYWORDS = {"from","import","def","class","return","await","async","if","else","elif",
             "for","while","with","as","try","except","finally","raise","yield","not",
             "in","is","and","or","None","True","False","lambda","pass","global"}

def _code_runs(line, size, in_doc=False):
    """Tokenise ONE line of Python into coloured runs (comment / string /
    keyword / call / number / plain). Deliberately simple — enough to make
    slide code readable, not a real parser.

    `in_doc` marks a line inside a triple-quoted block, which is coloured
    wholesale as a string (docstrings are prompts in this course, so they
    must read as one unit)."""
    import re as _re
    out=[]
    stripped=line.lstrip()
    if in_doc or stripped.startswith('"""') or stripped.startswith("'''"):
        return [(line,size,CODE_STR,False)]
    if stripped.startswith("#"):
        return [(line,size,CODE_CMT,False)]
    ...
    for i,ch in enumerate(line):
        if q:
            ...
            break
    for tok in _re.findall(r'"[^"]*"|\'[^\']*\'|\w+|\W', code):
        if not tok: continue
        if tok[0] in "\"'":                      col=CODE_STR
        elif tok in _KEYWORDS:                   col=CODE_KW
        elif tok.isdigit():                      col=CODE_NUM
        elif tok[0].isupper() and tok.isalnum(): col=CODE_FN
        else:                                    col=CODE_TXT
        out.append((tok,size,col,tok in _KEYWORDS))
    if comment: out.append((comment,size,CODE_CMT,False))
    return out or [(" ",size,CODE_TXT,False)]

def _code_lines(code,size):
    """Whole-block tokeniser: tracks triple-quoted docstrings across lines so a
    multi-line docstring renders as one continuous string colour."""
    out=[]; in_doc=False
    for line in code.split("\n"):
        fences=line.count('"""')+line.count("'''")
        if in_doc:
            out.append(_code_runs(line,size,in_doc=True))
            if fences: in_doc=False
        else:
            # a line that OPENS a docstring is itself string-coloured
            out.append(_code_runs(line,size,in_doc=(fences==1)))
            if fences==1: in_doc=True          # opened, not closed on this line
    return out

def _code_height(nlines,size):
    """Height the code card needs: line height ~1.34x the point size, plus padding.
    Shrink-wrapping matters — a fixed-height card under a 6-line snippet reads as
    a rendering bug."""
    return int(Inches(0.32) + Inches(nlines * size * 1.34 / 72.0))

def code_slide(title,code,kicker=None,accent=BLUE,note=None,size=None,caption=None):
    """Full-width syntax-highlighted code card, vertically centred and
    shrink-wrapped to its own line count."""
    s=head(slide(),title,kicker,kcolor=accent)
    lines=code.split("\n"); n=len(lines)
    if size is None:
        size = 15 if n<=14 else (13 if n<=18 else (11.5 if n<=23 else 10))
    area_top=Inches(1.95); area_h=Inches(4.4 if (note or caption) else 4.85)
    h=min(_code_height(n,size),area_h)
    top=int(area_top+(area_h-h)/2)                    # centre it in the content area
    rect(s,Inches(0.85),top,Inches(11.63),h,CODE_BG)
    rect(s,Inches(0.85),top,Inches(0.09),h,accent)
    txt(s,Inches(1.2),top+Inches(0.16),Inches(11.1),h-Inches(0.3),
        _code_lines(code,size),space=1)
    if caption:
        rect(s,Inches(0.85),Inches(6.45),Inches(11.63),Inches(0.55),LIGHT)
        txt(s,Inches(1.1),Inches(6.45),Inches(11.2),Inches(0.55),
            [[(caption,12.5,INK,False)]],anchor=MSO_ANCHOR.MIDDLE)
    elif note:
        txt(s,Inches(0.85),Inches(6.45),Inches(11.63),Inches(0.55),
            [[(note,13,GREY,False)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    footer(s); return s

def code_points(title,code,points,kicker=None,accent=BLUE,size=None,cw_in=6.9):
    """Code card on the LEFT, colour-coded takeaway tiles on the RIGHT.
    The workhorse for 'here is the API, here is what matters about it'."""
    s=head(slide(),title,kicker,kcolor=accent)
    lines=code.split("\n"); n=len(lines)
    if size is None:
        size = 12.5 if n<=14 else (11 if n<=19 else 9.5)
    area_top=Inches(1.95); area_h=Inches(4.78); cw=Inches(cw_in)
    h=min(_code_height(n,size),area_h)
    top=int(area_top+(area_h-h)/2)                    # shrink-wrap + centre
    rect(s,Inches(0.85),top,cw,h,CODE_BG)
    rect(s,Inches(0.85),top,Inches(0.08),h,accent)
    txt(s,Inches(1.15),top+Inches(0.14),cw-Inches(0.45),h-Inches(0.28),
        _code_lines(code,size),space=1)
    rx=int(Inches(0.85)+cw+Inches(0.32)); rw=int(Inches(12.48)-rx)
    m=len(points); gy=Inches(0.18); th=int((area_h-gy*(m-1))/m)
    for i,(pt,sub) in enumerate(points):
        y=int(area_top+(th+gy)*i); col=PALETTE[i%len(PALETTE)]
        rect(s,rx,y,rw,th,LIGHT); rect(s,rx,y,Inches(0.09),th,col)
        txt(s,rx+Inches(0.26),y,rw-Inches(0.48),th,
            [[(pt,13.5,col,True)],[(sub,11.5,INK,False)]],anchor=MSO_ANCHOR.MIDDLE,space=2)
    footer(s); return s

def annotated_code(title,code,annotations,kicker=None,accent=VIOLET,note=None):
    """Code card with numbered callout badges down the left gutter, and the
    matching numbered explanations underneath. annotations: list of strings —
    badge i is drawn beside code line annotations[i][0] (1-based)."""
    s=head(slide(),title,kicker,kcolor=accent)
    lines=code.split("\n"); n=len(lines)
    size = 13 if n<=12 else (11.5 if n<=16 else 10)
    top=Inches(1.95)
    lh=Inches(size*1.34/72.0)                    # one rendered line
    h=int(Inches(0.28)+lh*n)                     # shrink-wrap to the code
    rect(s,Inches(1.35),top,Inches(11.13),h,CODE_BG)
    txt(s,Inches(1.7),top+Inches(0.14),Inches(10.6),h-Inches(0.28),
        _code_lines(code,size),space=1)
    bd=Inches(0.34)
    for i,(ln,_t) in enumerate(annotations):
        # centre the badge on the line it annotates (ln is 1-based)
        y=int(top+Inches(0.14)+lh*(ln-1)+lh/2-bd/2)
        col=PALETTE[i%len(PALETTE)]
        oval(s,Inches(0.9),y,bd,bd,col)
        txt(s,Inches(0.9),y,bd,bd,[[(str(i+1),12,WHITE,True)]],
            align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    m=len(annotations); cols=2; rows=math.ceil(m/cols)
    X0=Inches(0.85); Y0=int(top+h+Inches(0.28)); TOTW=Inches(11.63)
    gx=Inches(0.3); gy=Inches(0.14)
    avail=max(Inches(0.9), Inches(6.8)-Y0)        # room left above the footer
    cwid=int((TOTW-gx*(cols-1))/cols)
    chgt=int((avail-gy*(rows-1))/rows) if rows else avail
    chgt=min(chgt,Inches(0.78))                   # keep rows tight, not stretched
    for i,(_ln,text) in enumerate(annotations):
        r=i//cols; c=i%cols
        x=int(X0+(cwid+gx)*c); y=int(Y0+(chgt+gy)*r); col=PALETTE[i%len(PALETTE)]
        bd2=Inches(0.3)
        oval(s,x,int(y+chgt/2-bd2/2),bd2,bd2,col)
        txt(s,x,int(y+chgt/2-bd2/2),bd2,bd2,[[(str(i+1),11,WHITE,True)]],
            align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
        txt(s,x+Inches(0.42),y,cwid-Inches(0.5),chgt,[[(text,12,INK,False)]],
            anchor=MSO_ANCHOR.MIDDLE)
    if note:
        txt(s,Inches(0.85),Inches(6.85),Inches(11.63),Inches(0.32),
            [[(note,12,GREY,False)]],align=PP_ALIGN.CENTER)
    footer(s); return s

def api_table(title,rows,kicker=None,accent=BLUE,note=None,headers=("Parameter","What it does")):
    """API/parameter reference: a monospace name column beside a plain-language
    description column. Alternating row fill, header band in the accent colour."""
    s=head(slide(),title,kicker,kcolor=accent)
    X0=Inches(0.85); TOTW=Inches(11.63)
    w0=int(TOTW*0.30); w1=TOTW-w0
    area=Inches(4.62) if note else Inches(4.85)
    nrow=len(rows)+1; rh=int(area/nrow); y=Inches(1.95)
    rect(s,X0,y,w0,rh,accent); rect(s,X0+w0,y,w1,rh,accent)
    txt(s,X0+Inches(0.16),y,w0-Inches(0.3),rh,[[(headers[0],13,WHITE,True)]],anchor=MSO_ANCHOR.MIDDLE)
    txt(s,X0+w0+Inches(0.16),y,w1-Inches(0.3),rh,[[(headers[1],13,WHITE,True)]],anchor=MSO_ANCHOR.MIDDLE)
    fs = 12 if len(rows)<=8 else 10.5
    for i,(name,desc) in enumerate(rows):
        yy=int(Inches(1.95)+rh*(i+1)); fill=LIGHT if i%2==0 else WHITE
        rect(s,X0,yy,w0,rh,fill,line=LINE); rect(s,X0+w0,yy,w1,rh,fill,line=LINE)
        tb=s.shapes.add_textbox(X0+Inches(0.16),yy,w0-Inches(0.3),rh)
        tf=tb.text_frame; tf.word_wrap=True; tf.vertical_anchor=MSO_ANCHOR.MIDDLE
        r=tf.paragraphs[0].add_run(); r.text=name
        r.font.size=Pt(fs); r.font.bold=True; r.font.name="Consolas"
        r.font.color.rgb=accent
        txt(s,X0+w0+Inches(0.16),yy,w1-Inches(0.3),rh,[[(desc,fs-0.5,INK,False)]],anchor=MSO_ANCHOR.MIDDLE)
    if note:
        txt(s,Inches(0.85),Inches(6.62),Inches(11.7),Inches(0.4),
            [[(note,12.5,GREY,False)]],align=PP_ALIGN.CENTER)
    footer(s); return s

def compare3(title,cols,kicker=None,note=None):
    """Three labelled comparison columns of short rows — for
    'CrewAI vs AutoGen vs ADK' style slides. cols: [(name, colour, [rows])]."""
    s=head(slide(),title,kicker)
    X0=Inches(0.85); TOTW=Inches(11.63); gap=Inches(0.26); n=len(cols)
    cw=int((TOTW-gap*(n-1))/n)
    top=Inches(1.95); h=Inches(4.4) if note else Inches(4.8)
    for i,(name,col,rows) in enumerate(cols):
        x=int(X0+(cw+gap)*i)
        rect(s,x,top,cw,h,LIGHT)
        rect(s,x,top,cw,Inches(0.56),col)
        txt(s,x,top,cw,Inches(0.56),[[(name,15,WHITE,True)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
        ry=int(top+Inches(0.72)); rh=int((h-Inches(0.86))/max(len(rows),1))
        for j,row in enumerate(rows):
            yy=int(ry+rh*j)
            if j%2==0: rect(s,x+Inches(0.1),yy,cw-Inches(0.2),rh-Inches(0.04),WHITE)
            txt(s,x+Inches(0.24),yy,cw-Inches(0.48),rh,[[(row,11.5,INK,False)]],anchor=MSO_ANCHOR.MIDDLE)
    if note:
        txt(s,Inches(0.85),Inches(6.45),Inches(11.7),Inches(0.5),
            [[(note,13,GREY,False)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    footer(s); return s

def stack(title,items,kicker=None,accent=BLUE,note=None):
    """Vertical numbered stack — big badge, bold title, grey caption per row.
    Reads as a sequence (a process, a hierarchy, an ordered checklist)."""
    s=head(slide(),title,kicker,kcolor=accent)
    n=len(items); top=Inches(1.95)
    area=Inches(4.45) if note else Inches(4.8)
    gy=Inches(0.16); rh=int((area-gy*(n-1))/n)
    for i,(t1,t2) in enumerate(items):
        y=int(top+(rh+gy)*i); col=PALETTE[i%len(PALETTE)]
        rect(s,Inches(0.85),y,Inches(11.63),rh,LIGHT)
        rect(s,Inches(0.85),y,Inches(0.09),rh,col)
        bd=min(Inches(0.62),rh-Inches(0.12))
        oval(s,Inches(1.15),int(y+rh/2-bd/2),bd,bd,col)
        txt(s,Inches(1.15),int(y+rh/2-bd/2),bd,bd,[[(str(i+1),17,WHITE,True)]],
            align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
        txt(s,Inches(2.0),y,Inches(10.3),rh,
            [[(t1,14.5,INK,True),("   —   ",13,LINE,False),(t2,12.5,GREY,False)]],
            anchor=MSO_ANCHOR.MIDDLE)
    if note:
        txt(s,Inches(0.85),Inches(6.5),Inches(11.63),Inches(0.45),
            [[(note,13,GREY,False)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    footer(s); return s

def split_note(title,left_head,left_items,right_head,right_items,kicker=None,
               lcolor=RED,rcolor=TEAL,note=None):
    """Do / Don't (or Wrong / Right) two-panel with tinted backgrounds."""
    s=head(slide(),title,kicker)
    top=Inches(1.95); h=Inches(4.3) if note else Inches(4.7)
    rect(s,Inches(0.85),top,Inches(5.7),h,RGBColor(0xFE,0xF2,0xF2) if lcolor==RED else LIGHT)
    rect(s,Inches(6.95),top,Inches(5.55),h,RGBColor(0xE8,0xF7,0xEE) if rcolor==TEAL else LIGHT)
    rect(s,Inches(0.85),top,Inches(5.7),Inches(0.5),lcolor)
    rect(s,Inches(6.95),top,Inches(5.55),Inches(0.5),rcolor)
    txt(s,Inches(0.85),top,Inches(5.7),Inches(0.5),[[(left_head,14.5,WHITE,True)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(6.95),top,Inches(5.55),Inches(0.5),[[(right_head,14.5,WHITE,True)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    bullets(s,Inches(1.15),top+Inches(0.72),Inches(5.15),h-Inches(0.9),left_items,size=13,gap=8)
    bullets(s,Inches(7.25),top+Inches(0.72),Inches(5.0),h-Inches(0.9),right_items,size=13,gap=8,mcolor=rcolor)
    if note:
        txt(s,Inches(0.85),Inches(6.4),Inches(11.63),Inches(0.5),
            [[(note,13,GREY,False)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    footer(s); return s

def verify_slide(a):
    """The acceptance criterion for a hands-on activity — the check that decides
    whether the learner may move on. Rendered as a green 'Test it' panel plus the
    lab folder it belongs to."""
    s=head(slide(),f"Activity {a['num']} — Test It",
           kicker=f"VERIFY BEFORE YOU MOVE ON",kcolor=TEAL)
    rect(s,Inches(0.85),Inches(2.05),Inches(11.63),Inches(2.75),RGBColor(0xE8,0xF7,0xEE))
    rect(s,Inches(0.85),Inches(2.05),Inches(0.1),Inches(2.75),TEAL)
    txt(s,Inches(1.25),Inches(2.3),Inches(11),Inches(0.45),
        [[("\u2705  Test it",19,RGBColor(0x12,0x7A,0x3E),True)]])
    txt(s,Inches(1.25),Inches(2.9),Inches(10.9),Inches(1.7),[[(a["test"],15,INK,False)]])
    rect(s,Inches(0.85),Inches(5.05),Inches(11.63),Inches(0.82),LIGHT)
    txt(s,Inches(1.15),Inches(5.05),Inches(11.1),Inches(0.82),
        [[("YOU'LL PRODUCE   ",11.5,BLUE,True),(a["build"],12.5,INK,False)]],anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(0.85),Inches(6.08),Inches(11.63),Inches(0.75),
        [[("Lab folder:  ",12,GREY,True),
          (f"labs/lab-{a['num']:02d}-...  ",12,BLUE,False),
          ("\u00b7   README.md \u00b7 app.py \u00b7 requirements.txt",12,GREY,False)],
         [(f"Full step-by-step: Learner Guide, Activity {a['num']}",12,GREY,False)]],space=3)
    footer(s); return s
