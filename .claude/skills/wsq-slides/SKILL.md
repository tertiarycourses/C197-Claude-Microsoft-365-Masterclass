...
11. **Practice Exam slide — CERTIFICATION COURSES ONLY.** Only courses leading to a
    third-party **certification exam** (Microsoft, AWS, Google, Cisco, CompTIA, PMI,
    Oracle, etc.) have a practice exam. **Non-certification courses do NOT have one —
    omit the slide entirely for these.** Skills-based/application courses (Python,
    n8n, Excel, data analytics, vibe coding, digital marketing …) are non-certification
    even when technical: if there is no external certification exam a learner sits
    after the course, there is no practice exam. Do not invent a link, and do not add
    the slide "for completeness" — pointing learners at a page that does not exist is
    worse than omitting it.

    For a certification course, search for the course's associated practice exam at
...
16. Practice Exam (CERTIFICATION courses only — omit for non-certification courses)
